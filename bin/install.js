...
function enableAutoUpdate(dest) {
  ...
  if (JSON.stringify(settings.hooks.SessionStart).includes("update-check.js"))
    return "already";
...
function reportDeps() {
  try {
    ...
    try {
      ...
      pluginEnabled = Object.keys(plugins).some(
        (k) => k.startsWith("superpowers@") && plugins[k],
      );
    } catch {}
    const hasSkill = (name) =>
      pluginEnabled ||
    ...
    console.log(
      "  Optional assists (used by the agents if present; everything degrades gracefully):",
    );
    if (missing.length === 0) {
      ...
      console.log(
        "      Install the superpowers plugin in Claude Code:  /plugin  →  claude-plugins-official",
      );
    }
    console.log(
      "    ℹ react-doctor (React projects only):  npx react-doctor@latest install",
    );
    try {
      ...
      console.log(
        "    ✓ codex CLI — found (optional Codex delegation available at scaffold)",
      );
    } catch {
      console.log(
        "    ℹ codex CLI (optional Codex delegation): not found — https://github.com/openai/codex",
      );
    }
    console.log("");
  } catch {}
...
Usage:  npx github:AlaskanTuna/pm-workflow [options]
  ...
  if (args.check) {
    ...
    console.log(
      `pm-workflow — installed: ${inst || "not installed"} | available: ${v}`,
    );
    return;
  }
  ...
  if (isGlobal && wantAuto === null && !args.yes) {
    const a = await ask(
      "  Enable auto-update? Adds a SessionStart hook that checks GitHub ~daily and updates this skill. [y/N]: ",
    );
  ...
  if (wantAuto === true && isGlobal) {
    ...
    const msg = {
      ...
      invalid:
        "  Auto-update:  skipped — ~/.claude/settings.json is not valid JSON; add the hook manually.",
      error:
        "  Auto-update:  skipped — could not write settings.json; add the hook manually.",
    }[r];
    console.log(msg);

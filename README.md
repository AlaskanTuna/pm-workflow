...
- 🤝 **Codex delegation (opt-in, per-feature, hardened)** — when the main agent is Claude, pick any combination of three independent features: `second-opinion` (a read-only review alongside QA with both verdicts at Gate 2, disagreements highlighted), `peer-consult` (a **blind** planning consult for high-stakes tasks — planner and Codex get the same brief independently, neither sees the other's answer, and the planner synthesizes both with the disagreement map shown at Gate 1), and `executor` (Codex workers implementing tasks under the full PG contract, with self-contained briefs, a PM-disclaimer preamble so workers never mistake PM orchestration rules for their own, and sandbox network access enabled when the task needs it). Every `codex exec` call follows a hardened invocation contract — pinned model + reasoning effort, closed stdin (the documented non-TTY freeze bug), hard timeout, JSON-event liveness monitoring, and prose-safe artifact placement that can't turn the lint gate red — so a hung Codex never stalls the pipeline.

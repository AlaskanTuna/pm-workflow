...
3. **Ask the human (use AskUserQuestion, ≤4 questions per call)** only for what you couldn't detect. Ask **visibility first** — it changes how everything else is written:
   ...
   - **Model profile** (routes each role's model; **effort is always pinned**, on every profile): `max` — planner Opus/max, programmer Sonnet/high, QA Opus/high (strongest reasoning at the bookends) · `balanced` — planner Opus/max, programmer + QA Sonnet/high · `economy` — all Sonnet, planner still max effort. **Recommend `max` when the human's plan has reliable Opus access; recommend `balanced` or `economy` for Pro-tier or rate-limited-Opus plans** — there, hard-pinning Opus would stall the pipeline or silently degrade it, whereas max-effort Sonnet planning still meaningfully beats default-effort everything. Recorded into `AGENTS.md`.
...
5. **Copy templates into place.** Never overwrite an existing file blindly; collisions follow the visibility mode:
   ...
   - `templates/agents/{planner,programmer,qa}.md` → `.claude/agents/`. Copy `programmer.md` verbatim; in `planner.md` and `qa.md` set the `model:` line from the chosen **model profile** — replace the quoted placeholder `"{{PLANNER_MODEL}}"` / `"{{QA_MODEL}}"` (quotes and all) with a bare model so the line reads e.g. `model: opus` (`max`: opus + opus · `balanced`: opus + sonnet · `economy`: sonnet + sonnet). Leave every `effort:` line exactly as written — effort is pinned on all profiles.
...
6. **Write the local excludes.** Skip entirely if the project isn't a git repo (no `.git/`) — tell the human to redo this step after `git init`. Append a marked block (`# --- pm-workflow ---` … `# --- /pm-workflow ---`) to `.git/info/exclude` (create it if needed; never use `.gitignore` for this — `exclude` is itself never committed, so it leaves no trace):
   - **Private mode:** one line per artifact the scaffold actually created (skip anything that pre-existed): `/AGENTS.md`, `/CLAUDE.md`, `/.claude/agents/planner.md`, `/.claude/agents/programmer.md`, `/.claude/agents/qa.md`, `/.claude/settings.local.json`, `/docs/roles.md`, `/docs/plan.md`, `/docs/progress.md`, `/docs/test.md`, `/docs/decisions.md`, `/docs/.pm-handoff.md` — plus `/.claude/CLAUDE.md` if the collision fallback was used.
...
8. **Check skill dependencies, then PAUSE if any are missing.** Check which referenced skills are present (this session's available skills, `~/.claude/skills/`, installed plugins):
   ...
   - **CLI tool (all roles):** `rtk` (command-output compression); check with `which rtk`, count as missing if absent. `graphify` is **optional** (Tier 2): use it when it is already installed (via its `AGENTS.md` block), but do NOT offer to install it here — building its knowledge graph has a one-time token cost the human may not want.

   Build the missing-set: the absent `superpowers` skills, plus `rtk` and (React only) `react-doctor` if it's absent. If the set is empty, continue silently. **Otherwise STOP and ask the human** (AskUserQuestion) with two options — do not proceed until they answer:
   - **Install them first** — tell them exactly how: `superpowers` via `/plugin` (marketplace `claude-plugins-official`); `react-doctor` via `npx react-doctor@latest install`; `rtk` via `curl -fsSL https://raw.githubusercontent.com/rtk-ai/rtk/refs/heads/master/install.sh | sh` (or `cargo install --git https://github.com/rtk-ai/rtk`) then `rtk init -g`. The plugin activates in the fresh Phase-B session (same restart the scaffold already requires), so skills and agents come online together. Then continue.
   ...
   You **cannot** install the `superpowers` plugin yourself — that is a human action that touches protected config. But `rtk` is an ordinary CLI install (not protected config): with the human's ok you may run its install command yourself. **Ordering:** AGENTS.md is filled in Step 5 from the pre-check tool state, so if you install `rtk` here, re-open the generated AGENTS.md and re-add its instructions block from `templates/AGENTS.md` (copy the lines between its matching instruction markers) when Step 5 had dropped it; if the human declines, leave it dropped. Present the choice and wait.
...
1. **Diff the verbatim files** against the current templates: `docs/roles.md` (ignore the version-stamp first line), `.claude/agents/{planner,programmer,qa}.md` (ignore the `model:` line in `planner.md`/`qa.md` — it reflects the project's chosen **model profile**, not template drift; diff the rest normally), `.claude/settings.local.json`, and the root `CLAUDE.md` adapter. These are the only upgrade candidates — **never touch** `AGENTS.md` (one exception: updating its single `Model profile:` line if the human switches profiles below), `docs/plan.md`, `docs/progress.md`, `docs/test.md`, or `docs/decisions.md` (they hold project/user content).
   - **Model profile:** offer to switch it (`max` / `balanced` / `economy`). On a switch, re-fill the `model:` line in `.claude/agents/planner.md` and `qa.md` from the new profile and update the `Model profile:` line in `AGENTS.md` — nothing else. Leave every `effort:` line untouched. Agent-model changes take effect only after a **session restart** — remind the human.
   - **Missing decision log:** if `docs/decisions.md` doesn't exist (older scaffold), offer to add it from `templates/decisions.md`; in private mode also add `/docs/decisions.md` to `.git/info/exclude`.
...
- **Right model, right role:** each role's model + effort follow the project's **model profile** (recorded in `AGENTS.md`; the agent frontmatter was filled from it at scaffold). Planning leads with **max effort**, the programmer runs Sonnet/high, QA reviews at high effort — planning errors are the costliest to unwind, so the bookends get the strongest reasoning. On `max` those bookends are Opus; on `balanced`/`economy` they step down to Sonnet while **effort stays pinned** (max-effort Sonnet planning still beats default-effort everything). Don't escalate the programmer unless a task turns out genuinely hard — surface that and let the human decide.

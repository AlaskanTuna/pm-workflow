...
7. **Confirm, then STOP — do not run a task in this session.** Show the file tree you created and a 4-line pipeline summary. Then tell the human to open a **brand-new session at the project root**: `/exit`, then `cd` into the project directory (**if you scaffolded into a subdir, `cd` into that subdir**) and start a fresh `claude` session there. **Resuming the same chat does not work** — it keeps the stale agent registry and the named agents won't be found.

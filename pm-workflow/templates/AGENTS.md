...
- **Model profile:** `{{MODEL_PROFILE — max | balanced | economy}}` — routes each role's model; **effort is always pinned**. `max`: planner Opus/max, programmer Sonnet/high, QA Opus/high · `balanced`: planner Opus/max, programmer + QA Sonnet/high · `economy`: all Sonnet, planner still max effort. The `.claude/agents/*.md` frontmatter is filled from this at scaffold; switch it via the upgrade flow.
...
- **Codex delegation:** `{{CODEX_MODE — off | second-opinion | executor | both; DELETE this line if the codex CLI is not installed}}` — **second-opinion** = a read-only `codex exec` review runs alongside QA (both verdicts reach Gate 2), plus an optional **blind peer consult** at planning for high-stakes tasks (human-triggered per task, never automatic); **executor** = Codex workers may implement PG tasks (same PG contract: no commits, surgical changes); **both** = both. The PM verifies the `codex` CLI before each use and degrades silently to Claude-only if it's missing.
